...
<?php
if(isset($_GET["kode"])){
require "resources/detail_penjualan.php";
}else{
...
<form class="form-search" style="float: right;" method="GET" action="page.php">
  ...
  <input type="hidden" name="page_name" value="surat_jalan" >
...
<tr>
  ...
  <?php
		...
		if(isset($_GET["q"])){
      $q = $_GET["q"];
      $sqlScript = "SELECT * FROM surat_jalan where kode_surat_jalan LIKE '%$q%' or kode_penjualan LIKE '%$q%'";
    }else{
      $sqlScript = "SELECT * FROM surat_jalan";
    }
	...
	?>
		<tr onclick="document.location='page.php?page_name=surat_jalan&kode=<?php echo $data["kode_surat_jalan"]; ?>'">
			<td><?php echo $no; ?></td>
